...
from agent_api.decision_claude import build_argv
...
def test_runner_injects_mcp_json_under_dot_claude(tmp_path):
    seed = tmp_path / "seed"; seed.mkdir(); (seed / "CLAUDE.md").write_text("conventions\n")
    captured: dict = {}

    class _Runner(SubprocessChatRunner):
        def _exec_claude(self, argv, cwd):
            captured["argv"] = argv
            yield json.dumps({"type": "result", "subtype": "success", "result": "ok", "session_id": "S1"})

    r = _Runner(str(tmp_path / "ws"), seed_dir=str(seed), tool_registry=_registry(tmp_path))
    list(r.run("triage", subject="u_jane", tools=["email"]))
