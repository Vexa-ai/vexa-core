"""Stage 2 (P20) — the gateway fronts the AGENT control plane (agent-api) under /api/*.
...
def test_invalid_key_is_401():
    client, _ = _client()
    r = client.get("/api/sessions", headers={"x-api-key": "nope"})
    assert r.status_code == 401
...
def test_nested_agent_path_and_query_carry_through():
    ...
    client, downstream = _client()
    client.get("/api/workspace/tree?hidden=1", headers=AUTH)
...
def test_client_supplied_user_id_is_stripped_then_reinjected():
    ...
    client, downstream = _client()
    client.post("/api/routines", headers={**AUTH, "x-user-id": "999"}, json={"name": "x"})
    assert downstream.last["headers"]["x-user-id"] == "7"
...
def test_agent_write_methods_forward():
    ...
    client, downstream = _client()
    client.put("/api/routines/daily/enabled", headers=AUTH, json={"enabled": True})
...
def test_chat_is_streamed_not_buffered_with_injected_user():
    ...
    r = client.post("/api/chat", headers=AUTH, json={"prompt": "hi", "session": "s1"})
    assert r.status_code == 200
...
def test_meeting_stream_is_streamed_with_injected_user():
    ...
    r = client.get("/api/meeting/stream?meeting_id=m1&session_uid=s1", headers=AUTH)
    assert r.status_code == 200
    assert r.headers["content-type"].startswith("text/event-stream")
