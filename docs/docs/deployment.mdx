...
- **Agent inference** — bring your own: Anthropic, OpenRouter, or a **self-hosted/local** model behind an
  Anthropic-compatible endpoint, so no inference leaves the network. Full recipes (incl. reusing a Claude
  subscription, and what it takes to swap the agent runner) in [How-to → Models & inference](/how-to/models).
