...
down) — and `pnpm gate:calm` ([Architecture as Code](/architecture/architecture-as-code)). The full P9
suite (readme · isolation · exports · graph · schema, plus contract/arch **sealing**) runs in the
upstream product.

<Note>The gate **runner** that orchestrates the full suite (`scripts/gates.mjs`, invoked as `pnpm gates`
/ `pnpm gate:*`) ships with the upstream product, not this open-core checkout. What is runnable here:
`pnpm typecheck build test`, the compose stack proof (`make -C deploy/compose stack-test`), `pnpm
gate:calm`, and each contract's `validate.mjs` / each brick's `check-isolation.js` run directly.</Note>
