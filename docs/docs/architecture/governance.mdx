...
- **Input trust** (from the trigger): **trusted** (you, in [chat](/capabilities/chat)) ⇒ the agent may
...
[Stream](/primitives/stream): `record` (a task/note — payload is the file), `draft`, `send` (external).
**Untrusted agent proposes → human approves → trusted code applies**: a `record` is committed by a trusted
applier; a `send` is executed by the [Integration](/primitives/integration). There is **no
workspace-structure/schema check** anywhere — the workspace is [just files](/primitives/workspace).
