...
```bash
curl -X POST "$API_BASE/agent/api/routines" \
...
```bash
# list
curl -H "X-API-Key: $API_KEY" "$API_BASE/agent/api/routines"

# pause / resume
curl -X PATCH "$API_BASE/agent/api/routines/Morning%20brief/enabled" \
  -H "X-API-Key: $API_KEY" -H "Content-Type: application/json" \
  -d '{"enabled":false}'

# delete (by routine id)
curl -X DELETE "$API_BASE/agent/api/routines/<routine_id>" -H "X-API-Key: $API_KEY"
```

## Read the result

```bash
curl -H "X-API-Key: $API_KEY" "$API_BASE/agent/api/workspace/file?path=brief/today.md"
```
