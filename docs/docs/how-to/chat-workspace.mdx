...
curl -X POST "$API_BASE/agent/api/chat/reset" -H "X-API-Key: $API_KEY" \
  -H "Content-Type: application/json" -d '{"session":"<sessionId>"}'      # → {"ok":true}

curl -H "X-API-Key: $API_KEY" "$API_BASE/agent/api/sessions"             # list sessions
curl -H "X-API-Key: $API_KEY" "$API_BASE/agent/api/sessions/<id>/history"
```
