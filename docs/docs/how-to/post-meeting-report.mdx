...
curl -H "X-API-Key: $API_KEY" "$API_BASE/agent/api/workspace/file?path=meetings/2026-06-29-acme.md"
```
...
`POST /api/meeting/start` then stream cards from `GET /api/meeting/stream`. See
