...
description: "Open-source and self-hosted: capture meetings and docs as Markdown, then let sandboxed AI agents turn them into actionable knowledge."
---
...
Real-time transcripts from Google Meet, Zoom, and Teams, Whisper included — and delivers to agents to build knowledge as code, fully selfhosted.
...
<CardGroup cols={2}>
  <Card title="Quickstart" icon="rocket" href="/deployment">
    Self-host the full stack with one `make` command. Up in minutes on a single Linux host.
  </Card>
...
- **Cloud-native deployment** — runs anywhere: a single container, [Docker Compose](/deployment), or [Kubernetes (Helm)](/deployment) — scale from a laptop to a cluster.
...
make lite     # single container, all services — for evaluation
```

The API comes up at `http://localhost:8056` (gateway) and the web UI at
`http://localhost:3000`. Full options — Docker Compose, Helm, air-gapped — in
