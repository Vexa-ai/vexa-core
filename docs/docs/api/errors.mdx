...
| `401` | no credentials — missing `X-API-Key` / `X-Admin-API-Key` | `Missing API Key` |
| `403` | bad or under-scoped credentials | `Invalid API Key` · `Token scope not authorized for this endpoint` |
| `404` | resource not found — user, token, meeting, or workspace file | `User not found` · `path not found` |
...
- `401` missing `X-Admin-API-Key`; `403` wrong admin token.
...
- `401`/`403` on a missing or invalid `X-API-Key`.
...
- Workspace reads → `404` if the path is absent (`GET /api/workspace/file?path=...`).
