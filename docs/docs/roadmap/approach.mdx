...
The roadmap is **derived from the [principles](/foundations/principles)**, not bolted on. Three rules
...
core re-grounded — see [the model](/foundations/the-model) and the principle *isolation is the
enforcement*.
</Note>
...
1. **Execution core** — [Runtime](/primitives/runtime) + generic [Agent](/primitives/agent) runner +
   [Workspace](/primitives/workspace) mount + [Stream](/primitives/stream). *Prove: one dispatch = one
...
3. **Triggers** — the [Scheduler](/primitives/scheduler) (time + event) and
   [Integrations](/primitives/integration). *Prove: a non-human launcher dispatches under a signed grant.*
4. **Capabilities** — chat, routines, integrations, [live meetings](/capabilities/live-meetings), org
