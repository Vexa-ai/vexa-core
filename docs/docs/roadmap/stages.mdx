...
<Note>
Format per stage — **Goal** · **Derives from** (a [principle](/foundations/principles)) · **Delivers** ·
...
- **Goal.** Turn the [seven primitives](/primitives/overview) into sealed, versioned schemas before any
  code.
...
- **Done when.** Contracts sealed; goldens validate; `schema` + `contract-version` gates green; the
  [model](/foundations/the-model) and this site reference only these shapes.
...
- **Delivers.** the agent worker entrypoint (generic over the [runner](/primitives/agent)); agent-api →
  [Runtime](/primitives/runtime) `POST /workloads` for **every** dispatch; the
  [workspace mount](/architecture/execution); the [Stream](/primitives/stream) relay (chat over
  `unit:<id>:out`).
- **Depends on.** Stage 0.
...
- **Delivers.** the [Scheduler](/primitives/scheduler) (redis; schedule entry + event + now) and
  [Integrations](/primitives/integration) (inbound watcher → fire; outbound MCP tool). Post-meeting rides
...
- **Delivers.** [Chat](/capabilities/chat), [Routines](/capabilities/routines),
  [Knowledge](/capabilities/knowledge) views, [Integrations](/capabilities/integrations) (email/calendar),
  [Browser](/capabilities/browser), [Live meetings](/capabilities/live-meetings),
  [Org knowledge](/capabilities/org-knowledge).
