...
- **Helm-first production.** Kubernetes deploys via the `deploy/helm/charts/vexa` chart; the runtime
  spawns bots and agent workers as Pods (`runtime.backend=k8s`). See [Deploy to Kubernetes](/how-to/deploy-kubernetes).
...
(compose) or `global.imageTag` (Helm) so deploys are reproducible.
