...
Vexa is configured by environment variables, set in `deploy/compose/.env` for Docker Compose or as
Helm values for Kubernetes. Defaults below are what the stack falls back to when the variable is unset
— fine for local evaluation, **not** for anything exposed.
...
|---|---|---|
| `TRANSCRIPTION_SERVICE_URL` | — | STT service endpoint. Unset → bots join and record but produce no transcript. |
| `TRANSCRIPTION_SERVICE_TOKEN` | — | STT auth token (from `vexa.ai/account`, or your self-hosted GPU service). |
...
<Note>
`make lite` runs everything in one container and publishes a smaller, fixed set —
gateway `:8056`, dashboard `:3000`, terminal `:3001`, agent `:8100`. The variables above apply to the
multi-container compose stack (`make all`).
</Note>

## Kubernetes (Helm)

On Helm the same settings are Helm values (`global.imageTag`, `secrets.*`, `database.*`, `redisConfig.*`,
`runtime.backend=k8s`, …). Full values table and cookbook:
