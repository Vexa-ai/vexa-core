...
make lite        # single container, all services — first run prompts for your STT token
```
...
<Note>
`make lite` runs everything in one container and **auto-provisions an API key** for zero-login —
the UI just works. For the full multi-container stack (`make all`) and how to mint keys yourself,
...
<Steps>
  <Step title="Open the dashboard">
    Go to `http://localhost:3000`. You're already signed in to a self-host account.
  </Step>
...
Grab the auto-provisioned key, then send a bot and read the transcript:

```bash
# retrieve the key make lite minted for you
export API_KEY=$(docker exec vexa-lite sh -c 'cat /run/vexa/key.env' | cut -d= -f2)
...
<CardGroup cols={2}>
  ...
  <Card title="Deploy to production" icon="server" href="/deployment">
    Docker Compose, Kubernetes (Helm), and air-gapped.
  </Card>
</CardGroup>
