...
export function validateViewSource(source: string): ValidationResult {
  ...
  walk.ancestor(ast as never, {
    ImportDeclaration(node: AnyNode) {
      ...
      if (!ALLOWED_IMPORTS.has(src)) add(errors, node, `Import from "${src}" is not allowed. Use only the harness globals: React, ui, useMeeting, useActions, actions, useState, useMemo, useEffect.`);
    },
