...
export interface LiveSegment { speaker: string; text: string; t?: number }
...
function ensure(meetingId: string, sessionUid: string): Entry {
  ...
  es.onmessage = (m) => {
    let ev: { type?: string; speaker?: string; text?: string; t?: number; card?: LiveCard };
    try { ev = JSON.parse(m.data); } catch { return; }
    const s = e!.state;
    if (ev.type === "transcript") s.transcript.push({ speaker: ev.speaker ?? "?", text: ev.text ?? "", t: ev.t });
